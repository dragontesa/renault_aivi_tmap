#include "SettingEvChargeTypeViewMediator.h"
...
SettingEvChargeTypeViewMediator::SettingEvChargeTypeViewMediator( QObject* viewComponent )
    : AbstractViewMediator( viewComponent ),
      mChanged(false)
{
}

QList<int> SettingEvChargeTypeViewMediator::listNotificationInterests()
{
...
bool SettingEvChargeTypeViewMediator::onEvent( int event, const QVariant& data )
{
    T_DEBUGV( "event: %d", event );

    switch ( event ) {
    case Activated: {
            sendNotification(ZoneConstants::RegistSoftkey, ViewRegistry::objectName(ViewRegistry::SettingEvChargeTypeView));
    ...
    case SaveSetting: {
            ...
            sendNotification(SettingConstants::SavedSettingFromObjectName, ViewRegistry::objectName(ViewRegistry::SettingEvChargeTypeView));
        }
        break;
    case CloseView:
...
bool SettingEvChargeTypeViewMediator::handleNotification( const mvc::INotificationPtr& note )
{
    switch ( note->getId() ) {
    case SettingConstants::ApplySetting:
        {
            ...
            if ( section == SettingConstants::MapSubChargeType ||
                 section == SettingConstants::SearchOption) {
    ...
    case ZoneConstants::DoSoftkeyIndexMenuFunction:{
            ...
            if (objectName != ViewRegistry::objectName(ViewRegistry::SettingEvChargeTypeView))
                return false;
