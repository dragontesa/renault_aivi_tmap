#include "SettingGpsInfoViewMediator.h"
...
SettingGpsInfoViewMediator::SettingGpsInfoViewMediator( QObject* viewComponent )
    : AbstractViewMediator( viewComponent )
{
}

QList<int> SettingGpsInfoViewMediator::listNotificationInterests()
{
...
bool SettingGpsInfoViewMediator::onEvent( int event, const QVariant& data )
{
    T_DEBUGV( "event: %d", event );

    switch ( event ) {
    case Activated: {
            sendNotification(ZoneConstants::RegistSoftkey, ViewRegistry::objectName(ViewRegistry::SettingGpsInfoView));
    ...
    case RequestSetting: {
            QVariantMap body;
            body.insert( CommonConstants::Type, NavigationControllerProtocol::GetGPSInfo );
...
bool SettingGpsInfoViewMediator::handleNotification( const mvc::INotificationPtr& note )
{
    switch ( note->getId() ) {
    ...
    case NavigationControllerConstants::ResponseController: {
            QVariantMap data = note->getBody().value<QVariantMap>();
            if (data.contains(SettingConstants::GPSInfoMode)) {
                QQmlProperty::write( mViewComponent, CommonConstants::Settings, QVariant::fromValue( data ) );
    ...
    case ZoneConstants::DoSoftkeyIndexMenuFunction:{
            ...
            if (objectName != ViewRegistry::objectName(ViewRegistry::SettingGpsInfoView))
                return false;
