#include "SettingGuidViewMediator.h"
...
SettingGuidViewMediator::SettingGuidViewMediator( QObject* viewComponent )
    : AbstractViewMediator( viewComponent ),
      mChanged(false)
{
}

QList<int> SettingGuidViewMediator::listNotificationInterests()
{
...
bool SettingGuidViewMediator::onEvent( int event, const QVariant& data )
{
    T_DEBUGV( "event: %d", event );

    switch ( event ) {
    case Activated: {
            sendNotification(ZoneConstants::RegistSoftkey, ViewRegistry::objectName(ViewRegistry::SettingGuidView));
...
bool SettingGuidViewMediator::handleNotification( const mvc::INotificationPtr& note )
{
    switch ( note->getId() ) {
    case SettingConstants::ApplySetting: {
            ...
            if ( section == SettingConstants::GuidMain ||
                 section == SettingConstants::GuidSubVoice  ||
    ...
    case SettingConstants::ChangedSetting: {
            ...
            if ( sect == SettingConstants::GuidMain && key == SettingConstants::RoadViolation ) {
                int value = data[CommonConstants::Setting].value<SettingDtoPtr>()->value().toInt();
                T_DEBUG( QString( "RoadViolation '%1'" ).arg( value ) );
            ...
            if ( sect == SettingConstants::GuidMain && key == SettingConstants::SafeDrivingSoundAtMute ) {
                int value = data[CommonConstants::Setting].value<SettingDtoPtr>()->value().toInt();
                T_DEBUG( QString( "SafeDrivingSoundAtMute '%1'" ).arg( value ) );
                QVariantMap body;
    ...
    case ZoneConstants::DoSoftkeyIndexMenuFunction:{
            ...
            if (objectName != ViewRegistry::objectName(ViewRegistry::SettingGuidView))
                return false;
