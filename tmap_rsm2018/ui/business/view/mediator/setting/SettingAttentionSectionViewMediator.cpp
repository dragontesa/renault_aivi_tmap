#include "SettingAttentionSectionViewMediator.h"
...
SettingAttentionSectionViewMediator::SettingAttentionSectionViewMediator( QObject* viewComponent )
    : AbstractViewMediator( viewComponent ),
      mChanged(false)
{
}

QList<int> SettingAttentionSectionViewMediator::listNotificationInterests()
{
...
bool SettingAttentionSectionViewMediator::onEvent( int event, const QVariant& data )
{
    T_DEBUGV( "event: %d", event );

    switch ( event ) {
    case Activated: {
            sendNotification(ZoneConstants::RegistSoftkey, ViewRegistry::objectName(ViewRegistry::SettingAttentionSectionView));
...
bool SettingAttentionSectionViewMediator::handleNotification( const mvc::INotificationPtr& note )
{
    switch ( note->getId() ) {
    case SettingConstants::ApplySetting:
        {
            ...
            if ( section == SettingConstants::GuidSubAttentionSection) {
    ...
    case ZoneConstants::DoSoftkeyIndexMenuFunction:{
            ...
            if (objectName != ViewRegistry::objectName(ViewRegistry::SettingAttentionSectionView))
                return false;
