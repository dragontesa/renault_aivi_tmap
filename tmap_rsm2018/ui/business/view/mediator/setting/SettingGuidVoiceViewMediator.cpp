#include "SettingGuidVoiceViewMediator.h"
...
SettingGuidVoiceViewMediator::SettingGuidVoiceViewMediator( QObject* viewComponent )
    : AbstractViewMediator( viewComponent )
{
}

QList<int> SettingGuidVoiceViewMediator::listNotificationInterests()
{
...
bool SettingGuidVoiceViewMediator::onEvent( int event, const QVariant& data )
{
    T_DEBUGV( "event: %d", event );

    switch ( event ) {
    case Activated: {
            sendNotification(ZoneConstants::RegistSoftkey, ViewRegistry::objectName(ViewRegistry::SettingGuidVoiceView));
...
bool SettingGuidVoiceViewMediator::handleNotification( const mvc::INotificationPtr& note )
{
    switch ( note->getId() ) {
    case SettingConstants::ApplySetting:
        {
            ...
            if ( section == SettingConstants::GuidSubVoice) {
    ...
    case SettingConstants::ChangedSetting: {
            QVariantMap body;
            body.insert( "type", NavigationControllerProtocol::ApplyVoice);
    ...
    case ZoneConstants::DoSoftkeyIndexMenuFunction:{
            ...
            if (objectName != ViewRegistry::objectName(ViewRegistry::SettingGuidVoiceView))
                return false;
