#include "SettingOilAreaViewMediator.h"
...
SettingOilAreaViewMediator::SettingOilAreaViewMediator( QObject* viewComponent )
    : AbstractViewMediator( viewComponent ),
      mChanged(false)
{
}

QList<int> SettingOilAreaViewMediator::listNotificationInterests()
{
...
bool SettingOilAreaViewMediator::onEvent( int event, const QVariant& data )
{
    T_DEBUGV( "event: %d", event );

    switch ( event ) {
    case Activated: {
            sendNotification(ZoneConstants::RegistSoftkey, ViewRegistry::objectName(ViewRegistry::SettingOilAreaView));
...
bool SettingOilAreaViewMediator::handleNotification( const mvc::INotificationPtr& note )
{
    switch ( note->getId() ) {
    ...
    case ZoneConstants::DoSoftkeyIndexMenuFunction:{
            ...
            if (objectName != ViewRegistry::objectName(ViewRegistry::SettingOilAreaView))
                return false;
