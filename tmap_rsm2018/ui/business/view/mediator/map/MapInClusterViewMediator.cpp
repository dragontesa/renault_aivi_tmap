#include "MapInClusterViewMediator.h"
#include "Constants.h"
...
#include "VSM_Navi.h"
#include "../common/mapviewconstants.h"
...
MapInClusterViewMediator::MapInClusterViewMediator( QObject* viewComponent )
    : AbstractViewMediator( viewComponent )
{
}

MapInClusterViewMediator::~MapInClusterViewMediator()
{
    mViewComponent->deleteLater();
    mViewComponent = nullptr;
}

QList<int> MapInClusterViewMediator::listNotificationInterests()
{
...
bool MapInClusterViewMediator::onEvent( int event, const QVariant& data )
{
    switch( event ) {
    case Mediated:
        {
#ifndef TMAP_CLUSTER_GMEM_TEST_WITH_NO_RENDERER
    #ifdef TMAP_CLUSTER_GMEM_TEST_PAUSE_RENDERER
            qDebug() << "[Cluster]createRenderer #1";
            QMetaObject::invokeMethod( mViewComponent, "createRenderer", Q_ARG(QVariant, true) );
    #else
            qDebug() << "[Cluster]createRenderer #2";
            QMetaObject::invokeMethod( mViewComponent, "createRenderer", Q_ARG(QVariant, false) );
    #endif
#endif
    ...
    case RendererCreated:
        {
            qDebug() << "[Cluster]RendererCreated #1";

#ifndef TMAP_CLUSTER_GMEM_TEST_WITH_NO_RENDERER
            qDebug() << "[Cluster]RendererCreated";
            auto m = data.toMap();
            mRenderer = m["renderer"].value<QObject*>();
            auto handle = qvariant_cast<void*>(mRenderer->property("nativeHandle"));
            int fps = ClusterConstants::MapViewExpectedFps;
            QMetaObject::invokeMethod( mRenderer, "setExpectedFps", Q_ARG(int, fps) );

#ifdef TMAP_CLUSTER_GMEM_TEST_HIDE_SKYBOX
            QMetaObject::invokeMethod( mRenderer, "skyVisibility", Q_ARG(QVariant, false) );
#endif

            VSMMap_SetSkyType(handle, VSM_SKY_CLUSTER);
            VSMMap_SetTheme(handle, VSM_THEME_CLUSTER);
            VSMNavi_SetSubMapSimulMode(true);
...
//            DebugUtils::updateMapTheme(TestConstants::CC, mRenderer, TestConstants::C_C_CS_C);
//            DebugUtils::updateMapTheme(TestConstants::CC, mRenderer, TestConstants::C_C_RLT);
//            DebugUtils::updateMapTheme(TestConstants::CC, mRenderer, TestConstants::C_C_RLTT);
            ...
            QTimer::singleShot(1, this, [handle, this]()
            {
                ...
                for (int i = 8, count = max3dAngles.size(); i < count; i++ ) {
                    ...
                    auto newAngle = oldAngle - (std::abs(oldAngle - base)) * scale;
...
bool MapInClusterViewMediator::handleNotification( const mvc::INotificationPtr& note )
{    
...
//            }
    ...
    case MapConstants::GlobalMapViewMoveModeChanged:
        {
            auto moveMode = (SKT::MapViewConstants::MapMoveMode)note->getBody().toInt();
            if ( moveMode == SKT::MapViewConstants::Navigation || moveMode == MapViewConstants::Simulator ) {
                QQmlProperty::write( mRenderer, "moveMode", moveMode );
            }
        }
        break;
    ...
    case TestConstants::UpdateMapTheme:
        {
            auto m = note->getBody().toMap();
            DebugUtils::updateMapTheme(TestConstants::CC, mRenderer, m["item"].toString());
        }
        break;
    }
