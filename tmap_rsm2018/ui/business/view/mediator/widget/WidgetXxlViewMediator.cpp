#include "WidgetXxlViewMediator.h"
#include "Constants.h"
...
WidgetXxlViewMediator::WidgetXxlViewMediator(  QObject* viewComponent )
    : AbstractViewMediator( viewComponent )
{
}

WidgetXxlViewMediator::~WidgetXxlViewMediator()
{
    qDebug() << "[Gadget]WidgetXxlViewMediator.cpp ~WidgetXxlViewMediator";
}

QList<int> WidgetXxlViewMediator::listNotificationInterests()
{
...
bool WidgetXxlViewMediator::onEvent( int event, const QVariant& data )
{
    switch ( event ) {
    case Mediated:
        {
...
//            sendNotification(MapConstants::ApplyMapTheme, m);
            ...
            sendNotification(MapConstants::RequestGlobalMapViewStatus); // 2019-12-20 lckxx fixed TMCRSM-4416
...
//            DebugUtils::updateMapTheme(TestConstants::GXXL, mRenderer, TestConstants::GXXL_CS_C);
//            DebugUtils::updateMapTheme(TestConstants::GXXL, mRenderer, TestConstants::GXXL_RLT);
//            DebugUtils::updateMapTheme(TestConstants::GXXL, mRenderer, TestConstants::GXXL_RLTT);
...
bool WidgetXxlViewMediator::handleNotification( const mvc::INotificationPtr& note )
{
    switch ( note->getId() ) {
    ...
    case RgConstants::UpdateLaneState:
        {
            if ( !mRgProxy->isSimulatorRunning() ) {
                WidgetUtils::handleLaneUpdate( WidgetConstants::SizeXXL, mViewComponent, note );
            }
        }
        break;
    ...
    case RgConstants::UpdateTrafficGuidance:
        {
            if ( !mRgProxy->isSimulatorRunning() ) {
                ...
                // 이 업데이트는 남은거리를 전체 경로 거리와 같게 하므로 시작할 때 카바타의 위치를 두번 이동시키는 이슈를 제기한다.
                // 처음 화면이 생성될 때 한번만 업데이트하고, 이후는 주 화면의 교통정보바 와 동일한 로직을 따른다.
...
//                    QQmlProperty::write( mRenderer, "viewMode", vsmMapMode );
    ...
    case TestConstants::UpdateMapTheme:
        {
//            auto m = note->getBody().toMap();
//            DebugUtils::updateMapTheme(TestConstants::GXXL, mRenderer, m["item"].toString());
        }
        break;
    ...
    case ApplicationConstants::ShowGadget:
        {
            ...
            if ( viewType == UIConstants::GadgetXXL ) {
                sendNotification(RgConstants::RequestRouteGuidanceState, getMediatorId());
            }
        }
        break;
    ...
    case ApplicationConstants::FocusToWidget:
        {
            ...
            if (instanceId == UIConstants::GadgetXXL) {
...
void WidgetXxlViewMediator::parseGpsTime(const QVariant &v)
{
...
void WidgetXxlViewMediator::updateLanguage()
{
...
void WidgetXxlViewMediator::updateTrafficGuidance(int routeId)
{
