...
//			메모리를 생성시 블럭으로 관리하고 해제시 참조만 삭제하고 이후 메모리 필요시 기존 메모리를 사용하는 구조

#ifndef __NDDS_MEMORY_POOL_H__
#define __NDDS_MEMORY_POOL_H__
...
static const std::size_t NDDS_DEFAULT_MEMORY_POOL_SIZE        = 1000 ;                          //!< Initial MemoryPool size (in Bytes)
static const std::size_t NDDS_DEFAULT_MEMORY_CHUNK_SIZE       = 128 ;                           //!< Default MemoryChunkSize (in Bytes)
static const std::size_t NDDS_DEFAULT_MEMORY_SIZE_TO_ALLOCATE = NDDS_DEFAULT_MEMORY_CHUNK_SIZE * 2 ; //!< Default Minimal Memory-Size (in Bytes) to Allocate.
 ...
 */
class CNddsMemoryPool : public IMemoryBlock
{
  public :
    /*!
	Contructor
	...
	*/
    CNddsMemoryPool(const std::size_t &sInitialMemoryPoolSize = NDDS_DEFAULT_MEMORY_POOL_SIZE,
		        const std::size_t &sMemoryChunkSize = NDDS_DEFAULT_MEMORY_CHUNK_SIZE,
				const std::size_t &sMinimalMemorySizeToAllocate = NDDS_DEFAULT_MEMORY_SIZE_TO_ALLOCATE,
				bool bSetMemoryData = false) ;
    virtual ~CNddsMemoryPool() ; //!< Destructor
  ...
  private :
	...
	*/
	...
	TByte *m_ptrNewMemBlock;
	SMemoryChunk *m_ptrNewChunks;
