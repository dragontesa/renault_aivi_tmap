...
static const int NDDS_FREEED_MEMORY_CONTENT        = 0xAA ; //!< Value for freed memory
static const int NDDS_NEW_ALLOCATED_MEMORY_CONTENT = 0xFF ; //!< Initial Value for new allocated memory

/******************
Constructor
******************/
CNddsMemoryPool::CNddsMemoryPool(const std::size_t &sInitialMemoryPoolSize,
		                 const std::size_t &sMemoryChunkSize,
				         const std::size_t &sMinimalMemorySizeToAllocate,
				         bool bSetMemoryData)
{
	m_ptrNewMemBlock = NULL;
	m_ptrNewChunks = NULL;
...
Destructor
******************/
CNddsMemoryPool::~CNddsMemoryPool()
{
...
GetMemory
******************/
void *CNddsMemoryPool::GetMemory(const std::size_t &sMemorySize)
{
...
FreeMemory
******************/
void CNddsMemoryPool::FreeMemory(void *ptrMemoryBlock)
{
...
AllocateMemory
******************/
bool CNddsMemoryPool::AllocateMemory(const std::size_t &sMemorySize)
{
  ...
  //std::cerr << "Allocating Memory (Size : " << sBestMemBlockSize+uiNeededChunks * sizeof(SMemoryChunk) << ")...\n" ;
  ...
  m_ptrNewMemBlock = ptrNewMemBlock;
  m_ptrNewChunks = ptrNewChunks;
  ...
  if(m_bSetMemoryData)
  {
    memset(((void *) ptrNewMemBlock), NDDS_NEW_ALLOCATED_MEMORY_CONTENT, sBestMemBlockSize) ;
  }
...
******************/
unsigned int CNddsMemoryPool::CalculateNeededChunks(const std::size_t &sMemorySize)
{
...
******************/
std::size_t CNddsMemoryPool::CalculateBestMemoryBlockSize(const std::size_t &sRequestedMemoryBlockSize)
{
...
FreeChunks
******************/
void CNddsMemoryPool::FreeChunks(SMemoryChunk *ptrChunk)
{
  ...
  for(unsigned int i = 0; i < uiChunkCount; i++)
  {
    if(ptrCurrentChunk)
    {
	  ...
	  if(m_bSetMemoryData)
	  {
        memset(((void *) ptrCurrentChunk->Data), NDDS_FREEED_MEMORY_CONTENT, m_sMemoryChunkSize) ;
	  }
...
******************/
SMemoryChunk *CNddsMemoryPool::FindChunkSuitableToHoldMemory(const std::size_t &sMemorySize)
{
...
SkipChunks
******************/
SMemoryChunk *CNddsMemoryPool::SkipChunks(SMemoryChunk *ptrStartChunk, unsigned int uiChunksToSkip)
{
...
******************/
void CNddsMemoryPool::SetMemoryChunkValues(SMemoryChunk *ptrChunk, const std::size_t &sMemBlockSize)
{
...
******************/
bool CNddsMemoryPool::WriteMemoryDumpToFile(const std::string &strFileName)
{
...
LinkChunksToData
******************/
bool CNddsMemoryPool::LinkChunksToData(SMemoryChunk *ptrNewChunks, unsigned int uiChunkCount, TByte *ptrNewMemBlock)
{
...
******************/
bool CNddsMemoryPool::RecalcChunkMemorySize(SMemoryChunk *ptrChunk, unsigned int uiChunkCount)
{
  unsigned int uiMemOffSet = 0 ;
  for(unsigned int i = 0; i < uiChunkCount; i++)
  {
...
SetChunkDefaults
******************/
SMemoryChunk *CNddsMemoryPool::SetChunkDefaults(SMemoryChunk *ptrChunk)
{
  if(ptrChunk)
  {
    ptrChunk->Data = NULL ;
...
******************/
SMemoryChunk *CNddsMemoryPool::FindChunkHoldingPointerTo(void *ptrMemoryBlock)
{
...
FreeAllAllocatedMemory
******************/
void CNddsMemoryPool::FreeAllAllocatedMemory()
{
	if(m_ptrNewMemBlock)
	{
		free(m_ptrNewMemBlock);
		m_ptrNewMemBlock = NULL;
	}
...
DeallocateAllChunks
******************/
void CNddsMemoryPool::DeallocateAllChunks()
{
	if(m_ptrNewChunks)
	{
		free(m_ptrNewChunks);
...
IsValidPointer
******************/
bool CNddsMemoryPool::IsValidPointer(void *ptrPointer)
{
    SMemoryChunk *ptrChunk = m_ptrFirstChunk ;
	while(ptrChunk)
	{
...
MaxValue
******************/
std::size_t CNddsMemoryPool::MaxValue(const std::size_t &sValueA, const std::size_t &sValueB) const
{
  if(sValueA > sValueB)
  {
	  return sValueA ;
